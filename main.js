...
аутентификация - признание человека, что пользователь именно тот пользователь
авторизация    - выдача/невыдача доступа до определенных ресурсов

Раньше была сессионная аутентификация и авторизация
Клиент                      Сервер
1. Логин: логин/пароль ---------->      Клиент отправляет логин пароль
Сервер сравнивает, если всё ок то создает у себя сессию (кому принадлежит, дата начала, срок действия)
<------------ 2. Ответ: sessioт ID      В ответ на запрос в куках прописывает id сессии.
Куки путешествуют туда сюда и теперь в них есть это id
3. Запрос ресурса: sessioт ID --->      Когда запрашиваем доступ к ресурсу, смотри есть ли доступ у этого id
<------ 2. Ответ: доступ к ресурсу
Основная проблема: сервер вынужден хранить все сессии. 
А если у нас распределенная схема где куча микросервисов, нужно в каждой из них авторизововаться.
Мы не можем один раз авторизоваться и везде ходить по всем микросервисам.

Придумали JWT (джовт в оригинале, дживити в русском)
1. Логин: логин/пароль    ------------------>       Клиент отправляет логин пароль
<------2. Ответ: access_token и refresh_token       В ответ получил токены.
Теперь в запросах в заголовке будем отправлять токены
3. Запрос ресурса: по access_token --------->       При запросе теперь всегда отправляем access_token
Если токен протух (обычно 5мин, час) то
<------ Ошибка 401 или 403
Запрос на обновление: по refresh_token ----->
<------ Ответ: access_token и refresh_token         Получаем новую пару токенов
У refresh_token тоже есть время и если и он протух, то заново логинемся

Структура токена 
абракадабра.абракадабра.абракадабра
абракадабра         - строка закодированная в Base64 - представление любых данных (картинки, строки и т.д.)
btoa('qwerty')      - это команда раскодировает из Base64

Header.Payload.Signatue
Header      - строка в jsone ("{alg: "", typ: "JWT"}")  alg - алгоритм по которому зашифрован ключ
Payload     - строка в jsone ("{срок хранения токена и любые полезные данные по пользователю кроме пароля}")
Signatue    - это уже не Base64, а зашифрованный ключ по алгоритму из хедера.
Base64(Header).Base64(Payload).секретный_ключ
Таким образом на стороне сервера сравнение идет по этому ключу.

SSO (Single Sign-On)    один раз авторизовался и везде предъявляешь его

Токен могут украсть поэтому делаем небольшое время действия токена.

При разлогировании либо засовываем на сервере токен в черный список.
Либо удаляем токен из заголовка на стороне фронта.
*/
